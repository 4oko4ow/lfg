...
export function GameFeedClient({ slug, gameName }: Props) {
  ...
  const [filter, setFilter] = useState<string>(gameName);
  ...
  const gameCounts = useMemo(() => {
    ...
    parties.forEach((party) => {
      const s = party.game.toLowerCase();
  ...
  // Check for saved party to join after auth
  useEffect(() => {
  ...
  }, [parties, filter, ALL_LABEL, games, THREE_DAYS_MS, THREE_WEEKS_MS]);
                  ...
                  <span className="relative flex items-center gap-1.5">
                    ...
                    {loading ? (
                      <span className={`px-1.5 py-0.5 rounded-md ${
                        isSelected ? "bg-white/20" : "bg-zinc-700/60"
                      }`}>
                        <div className="h-3 w-4 animate-pulse rounded bg-zinc-600/50"></div>
                      </span>
                    ) : count > 0 ? (
                      <span className={`px-1.5 py-0.5 rounded-md text-[10px] font-bold ${
                        isSelected ? "bg-white/20 text-white" : "bg-zinc-700/60 text-zinc-400"
                      }`}>
                        {count}
                      </span>
                    ) : null}
