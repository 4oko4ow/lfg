...
| `AUTH_DB_PATH`              | No (defaults to `data/auth.json`) | File path for the JSON store that keeps linked identities and contact handles. **For Fly.io, set to `/data/auth.json` to use persistent volume.** | `data/auth.json` or `/data/auth.json` (Fly.io) |
| `AUTH_COOKIE_NAME`          | No (defaults to `lfg_session`)    | Overrides the session cookie name if you need a custom value.                                                                   | `fp_session`                                |
| `AUTH_COOKIE_DOMAIN`        | No                                | Cookie domain for production deployments. Leave empty for localhost. Set to `.findpaty.online` (with leading dot) to support both www and non-www subdomains. | `.findpaty.online`                         |
...
| `FRONTEND_URL`              | Yes                               | Base URL of the SPA used when redirecting back after OAuth flows.                                                               | `https://findpaty.online`                  |
| `BACKEND_URL`               | Yes                               | Public URL of the Go service, used for OAuth callbacks.                                                                         | `https://findpaty.online`                  |
